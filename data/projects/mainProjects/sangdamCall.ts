...
• **상담 세션 관리 및 Trial 사용자 제한**: 상담 내용 기록, 콜백 처리 등 상담 세션을 관리하고, \`Trial(체험)\` 사용자의 기능 접근을 제한하는 등 비즈니스 로직을 구현했습니다.`,
      ...
      solutions: [
        "**Zustand를 활용한 전역 상태 관리 아키텍처 도입**: \`Prop Drilling\`과 데이터 불일치 문제를 해결하기 위해 \`Zustand\` 기반의 전역 스토어를 도입했습니다. 현재 선택된 콜 정보와 같이 여러 컴포넌트가 공유해야 하는 핵심 데이터를 중앙에서 관리하여, 어떤 컴포넌트에서든 항상 최신 데이터를 일관되게 사용할 수 있도록 설계했습니다.",
        "**Intersection Observer 기반 무한 스크롤 구현**: 초기 로딩 성능을 최적화하기 위해, 첫 페이지의 데이터만 먼저 로드하고 사용자가 스크롤을 내릴 때 \`Intersection Observer\`가 이를 감지하여 다음 페이지 데이터를 비동기적으로 불러오는 무한 스크롤을 구현했습니다. 이를 통해 초기 로딩 시간을 대폭 단축하고 서버 부하를 감소시켰습니다.",
        "**Custom Hook을 통한 비즈니스 로직의 분리 및 추상화**: \`useInfiniteCallList\`와 같은 커스텀 훅을 만들어 API 연동, 데이터 정제, 무한 스크롤 상태 관리 등 복잡한 비즈니스 로직을 UI 컴포넌트로부터 완전히 분리했습니다. 이를 통해 컴포넌트는 오직 UI 렌더링에만 집중할 수 있게 하여 관심사 분리 원칙을 지켰고, 로직의 재사용성과 테스트 용이성을 높였습니다.",
      ],
      codeSnippets: [
        {
          title: "무한 스크롤 커스텀 훅",
...
• **모듈식 컴포넌트 아키텍처**: 데이터 시각화 컴포넌트(\`StatusDonutChart\`, \`CallStatsCard\` 등)를 기능 단위로 명확하게 분리하여 코드의 재사용성과 유지보수성을 높였고, 새로운 통계 위젯을 쉽게 추가할 수 있는 확장성을 확보했습니다.`,
      ...
      solutions: [
        "**Zustand 스토어 기반의 폴링 아키텍처 활용**: 데이터 호출 및 상태 관리 로직을 \`Zustand\` 스토어(\`dashboardStore\`)에 완전히 위임하는 폴링 아키텍처를 활용했습니다. 페이지 컴포넌트는 \`useEffect\`의 생명주기에 맞춰 스토어의 \`startPolling\`과 \`stopPolling\` 액션만 호출하며, 이를 통해 컴포넌트의 관심사를 명확히 분리하고 메모리 누수 없이 안정적인 실시간 데이터 업데이트를 구현했습니다.",
        "**상태 기반의 동적 API 호출 및 연산 최적화**: 사용자의 모든 필터 조건을 \`useState\`로 관리하고, \`useEffect\`를 통해 특정 상태의 변경을 감지하여 변경된 파라미터로 필요한 API만 동적으로 재호출하는 반응형 시스템을 구축했습니다. 또한, 차트 데이터 변환 등 계산 비용이 높은 연산은 \`useMemo\`로 메모이제이션하여 불필요한 반복 계산을 방지하고 애플리케이션의 반응성을 높였습니다.",
...
// ... JSX ...
    ...
    },
    {
      ...
      color: "purple",
      details: `파편화된 통계 데이터를 사용자가 원하는 조건으로 조회 및 시각화하는 **통합 시스템**을 구축했습니다. 프로젝트의 핵심은 복잡한 필터링과 \`API\` 요청을 안정적으로 관리하고, 프론트엔드 단에서 데이터를 효과적으로 가공 및 최적화하며, 전체 로직을 유지보수하기 용이한 아키텍처로 설계하는 것이었습니다.

• **복합 조건 필터링**: 다중 필터 조건에 따라 \`API\` 파라미터를 동적으로 구성합니다.
• **동적 뷰 전환 (UX 개선)**: 조회 기간에 따라 시간/일/월별 통계 뷰를 자동 제안합니다.
• **클라이언트 사이드 데이터 가공**: \`API\` 원본 데이터를 차트/테이블에 맞게 변환하고 '합계' 등 요약 정보를 생성합니다.
• **API 호출 최적화**: 필터 상태와 \`API\` 호출 상태를 분리하여 불필요한 서버 요청을 방지합니다.`,
      challenges: [
        ...
        "**백엔드와 프론트엔드의 데이터 구조 불일치**: 백엔드 API로부터 받은 원본 데이터는 종종 중첩된 객체 형태였으나, 프론트엔드의 차트 라이브러리나 테이블은 평탄화된 배열 형태의 데이터를 요구하여 그 간극을 메우는 효율적인 \`데이터 변환(Data Transformation) 계층\`이 필요했습니다.",
      ],
      solutions: [
        "**Zustand 스토어를 통한 상태 및 비즈니스 로직 중앙 관리**: 페이지의 UI 렌더링 책임과 데이터 관리 책임을 완벽하게 분리하는 아키텍처를 적용했습니다. 필터 조건, API 호출 함수, 로딩 상태, 원본 데이터, 가공 데이터 등 페이지에 필요한 모든 상태와 관련 로직을 Zustand 스토어에서 중앙 관리했습니다. 이를 통해 페이지 컴포넌트는 UI 렌더링에만 집중할 수 있게 하여 \`테스트 용이성과 유지보수성\`을 극대화했습니다.",
        "**useMemo를 활용한 데이터 변환 연산 최적화**: API 원본 데이터나 사용자가 선택한 뷰가 변경될 때만 차트와 테이블 데이터를 변환하는 로직이 실행되도록 useMemo로 메모이제이션(Memoization)을 적용했습니다. 이를 통해 불필요한 리렌더링 시마다 반복될 수 있는 무거운 데이터 가공 연산을 방지하고 애플리케이션의 \`성능과 반응성\`을 최적화했습니다.",
        "**데이터 처리 로직의 모듈화 및 추상화**: API 응답 데이터를 차트용, 테이블용 데이터로 가공하는 복잡한 로직을 별도의 유틸리티 파일(utils)로 분리하여 관리했습니다. \`map\`, \`reduce\`, \`Object.entries\`와 같은 고차 함수를 활용하여 데이터 변환 로직을 모듈화하고 추상화함으로써, 컴포넌트를 간결하게 유지하고 데이터 처리 로직을 \`독립적으로 테스트할 수 있도록 설계\`했습니다.",
      ],
      codeSnippets: [
        {
          title: "통계 페이지 아키텍처",
...
// ... JSX ...
    ...
    },
    {
      ...
      color: "red",
      details: `사용자가 직접 \`ARS\` 시나리오를 구성하고 목소리 설정을 변경할 수 있는 **설정 빌더(Builder) 페이지**를 개발했습니다. 이 프로젝트의 핵심은 '간편한 \`ARS\`'의 복잡하고 상호 의존적인 상태들을 **커스텀 훅 기반 아키텍처**로 분리하여 효과적으로 관리하고, '맞춤형 \`ARS\`'의 외부 전문 솔루션을 사용자에게 이질감 없이 매끄럽게 연동하는 것이었습니다.
...
• **외부 솔루션 연동 게이트웨이**: 복잡한 \`ARS\` 빌더는 외부 전문 솔루션(어썸봇)을 연동하는 방식으로 구현하여 개발 리소스를 절감하고 더 강력한 기능을 제공했습니다. 사용자가 외부 솔루션을 쉽게 사용하도록 안내하는 **게이트웨이 역할**을 수행합니다.
• **상태와 연동된 동적 UI** 및 비즈니스 규칙: \`ARS\` 연결 번호의 최대 개수를 초과하면 알림 모달을 띄우는 등 비즈니스 규칙을 상태 관리 로직과 결합했습니다. 또한, 전역 스토어의 사용자 \`ARS\` 타입에 따라 \`UI\`를 동적으로 제어하여 **데이터 정합성**을 유지합니다.
• **사용자 경험(UX) 중심의 안내 시스템**: 서비스 모드 변경이나 외부 솔루션 이동 전, 모달을 활용하여 사용자에게 명확한 정보를 제공하고 의사를 확인하는 **안정적인 UX 패턴**을 적용하여 사용자의 실수를 방지했습니다.`,
      challenges: [
        "**높은 상태 복잡도**와 컴포넌트 비대화: \`ARS\` 시나리오 설정과 목소리 설정은 서로 다른 도메인이지만, 하나의 페이지에서 관리되어야 했습니다. 이 모든 상태와 로직을 단일 컴포넌트에서 처리할 경우, 코드가 비대해지고 **유지보수가 불가능**해지는 문제가 있었습니다.",
        "**외부 솔루션과의 매끄러운 연동**: 사용자가 내부 서비스와 외부 솔루션 사이를 이동할 때 이질감을 느끼거나 혼란을 겪지 않도록, 명확한 사전 안내와 필수 조건 체크 등 세심한 **사용자 여정(User Journey) 설계**가 필요했습니다.",
        "**비즈니스 규칙과 UI 상태의 결합**: '연결 번호는 최대 7개까지만 추가 가능'과 같은 시스템 제약 사항을 \`UI\`에 자연스럽게 녹여내고, 사용자의 잘못된 조작을 효과적으로 방지하는 로직을 구현해야 했습니다.",
      ],
      solutions: [
        "**커스텀 훅을 통한 핵심 로직의 완전한 분리** 및 추상화: **관심사 분리(SoC) 원칙**에 따라, \`ARS\` 시나리오와 보이스 설정 로직을 각각의 커스텀 훅으로 완벽하게 분리했습니다. 이 아키텍처는 페이지 컴포넌트를 매우 간결하고 선언적으로 유지시켜주며, 각 훅은 독립적으로 개발하고 테스트할 수 있어 **코드의 안정성과 유지보수성**을 극대화했습니다.",
        "**게이트웨이 페이지 및 모달을 통한 UX 설계**: 외부 솔루션 연동을 위해, 기능을 직접 안내하고 필수 조건을 확인하는 **게이트웨이 페이지**를 설계했습니다. 또한, 서비스 모드 변경 등 중요한 작업 전에는 모달을 통해 사용자에게 명확한 정보를 제공하고 의사를 재확인하는 **안정적인 UX 패턴**을 적용하여 사용자의 혼란과 실수를 방지했습니다.",
        "**상수(Constants) 파일을 활용한 설정 중앙 관리**: \`ARS\` 연결 타입, 목소리 옵션 등 수많은 선택지를 별도의 상수 파일로 분리하여 중앙에서 관리했습니다. 이를 통해 **매직 넘버나 문자열을 제거**하여 코드의 가독성을 높이고, 향후 옵션 변경 시 **유지보수 비용을 크게 절감**했습니다.",
      ],
      codeSnippets: [
        {
